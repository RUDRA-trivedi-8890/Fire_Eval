...
• Tier 1: Per-Sentence Metrics
  ...
  avg_score = (CMI_norm + M-Index + I-Index + Fluency) / 4
...
def evaluate_row(index, tags, text, model, tokenizer, device):
    ...
    # Tier 1 Direct Balance (Fluency blended into the exact same average)
    avg_score = ((cmi / 100.0) + m_index + i_index + fluency_score) / 4
...
def summarize(name, results):
    ...
    file_blended_score = sum(r["avg_score"] for r in results) / n
    ...
    print(f"Fluency Avg     : {avg_flu:.4f}")
    print(f"Blended Score   : {file_blended_score:.4f}")

    return file_blended_score
...
def main():
    ...
    for filepath in input_files:
        ...
        if "hbe" in filename:
            hbe_results = process_file(filepath, model, tokenizer, device, "EN-HI-BN")
            hbe_score = summarize("HBE (Hindi-Bengali-English)", hbe_results)
            if hbe_score is not None:
                processed_count += 1
        elif "hge" in filename:
            hge_results = process_file(filepath, model, tokenizer, device, "EN-HI-GU")
            hge_score = summarize("HGE (Hindi-Gujarati-English)", hge_results)
            if hge_score is not None:
                processed_count += 1
        else:
            generic_results = process_file(filepath, model, tokenizer, device, "text")
            g_score = summarize(f"Generic Match ({os.path.basename(filepath)})", generic_results)
            if g_score is not None:
                hbe_score = g_score
                processed_count += 1

    if processed_count == 0:
        print("\nError: No valid JSONL datasets were processed.")
        sys.exit(1)

    # ──────────────────────────────────────────────────────────────────
    # TIER 2 SYSTEM-LEVEL AVERAGE (Works with 1 or 2 files seamlessly)
    # ──────────────────────────────────────────────────────────────────
    valid_scores = [s for s in [hbe_score, hge_score] if s is not None]
    final_combined_score = sum(valid_scores) / len(valid_scores)

    print("\n")
    print("=" * 60)
    print("FINAL COMBINED SUBTASK A LEADERBOARD SCORE")
    print("=" * 60)
    print(f"Processed Datasets        : {processed_count}")
    print(f"Final Integrated Score    : {final_combined_score:.4f}")
    print("=" * 60)

    # Dump the single absolute scalar dictionary item for Codabench leaderboards
    output_dir = '/output'
    if os.path.exists(output_dir):
        with open(os.path.join(output_dir, 'scores.json'), 'w') as f:
            json.dump({"score": round(final_combined_score, 4)}, f)
